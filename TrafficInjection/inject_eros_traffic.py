...
def main(argv):
    parser = argparse.ArgumentParser(
        description="Eros traffic injection script\n"
                    "In the receiver side, run: sudo ./inject_eros_traffic.py server 8123 tcp\n"
                    "In the sender side, run: ./inject_eros_traffic.py client 8123 tcp --server_address=serverip --traffic_profile=stream_workload0_2mbps_100s.json",
    ...
    if args.traffic_profile.lower() == "sink":
        ...
        def func_udp(traffic, clientsocket_and_address_tup):
            ...
            while n > 0:
                n -= 1
                clientsocket_and_address_tup[0].sendto(msg[:BUFFSIZE if n != 0 and r != 0 else r],
                                                       (clientsocket_and_address_tup[1], args.port))
...
def test():
    ...
    processes = [mp.Process(target=main, args=[("server",
                                                "8135",
                                                "udp",
                                                "--traffic_profile=echo")]),
                 mp.Process(target=main, args=[("client",
                                                "8135",
                                                "udp",
                                                "--server_address=127.0.0.1",
                                                "--traffic_profile=stream_workload0_2mbps_100s.json")])
                 ]
...
if __name__ == "__main__":
    # test() # Launch two processes that talk to each other
    main(sys.argv[1:])
